...
def main():
    ...
    for key, value in d.items():
        ...
        DeviceVersion.objects.create(name=key, target_id=value['targetId'], device=device)

    f = _req('GET', API_URL + '/firmwares')
    for key, value in f.items():
        device_version = DeviceVersion.objects.get(name=key)
        for fi in value:
            SeFirmwareVersion.objects.create(name=fi['name'],
                                             ...
                                             bolos_version_min=fi['bolos_version'].get('min', None),
                                             bolos_version_max=fi['bolos_version'].get('max', None),
    ...
    for key, value in a.items():
        ...
        for app_version in value:
            ...
            ApplicationVersion.objects.create(name=app_version['version'],
                                              notes=app_version.get('notes', None),
                                              icon=app_version.get('icon', None),
                                              delete=app_version['app'].get('delete', None),
                                              delete_key=app_version['app'].get('deleteKey', None),
                                              perso=app_version['app'].get('perso', None),
                                              hash=app_version['app'].get('hash', None),
                                              firmware=app_version['app'].get('firmware', None),
                                              firmware_key=app_version['app'].get('firmwareKey', None),
                                              target_id=app_version['app'].get('targetId', None),
                                              bolos_version_min=app_version['bolos_version'].get('min', None),
                                              bolos_version_max=app_version['bolos_version'].get('max', None),
                                              app=app,
                                              device_version=device_version)
